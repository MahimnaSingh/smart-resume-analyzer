...
# --- Robust YouTube title fetcher (replaces old fetch_yt_video) ---
@lru_cache(maxsize=256)
def _normalize_yt_url(link: str) -> str:
    """Return a full YouTube watch URL from either a full URL or a bare video id."""
    if not link:
        return ""
    link = link.strip()
...
def fetch_yt_video(link: str) -> str:
    """Return a title for a YouTube video link or id. Tries pafy first, then pytube, then fallback text.
    This function will never raise -- it returns a safe string on error."""
    full_url = _normalize_yt_url(link)
    if not full_url:
        return "Unknown YouTube Video"

    # Try pafy first (your existing dependency)
    try:
        import pafy

        video = pafy.new(full_url)
        title = getattr(video, "title", None)
        if title:
            return title
    except Exception:
        # pafy failed — fall back to pytube
        pass

    # Fallback to pytube (more robust)
    try:
        from pytube import YouTube

        yt = YouTube(full_url)
        if getattr(yt, "title", None):
            return yt.title
    except Exception:
        pass

    # Last-ditch: extract id and return readable fallback
    try:
        vid_id_match = re.search(r"v=([A-Za-z0-9_-]{11})", full_url)
        if vid_id_match:
            return f"YouTube Video ({vid_id_match.group(1)})"
    except Exception:
        pass
...
connection = pymysql.connect(host="localhost", user="root", password="943519")
